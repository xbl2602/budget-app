...
// Editor bounds. PLAN_YEAR_BACK/FWD size the dropdown around today; the wider
// PLAN_YEAR_MIN/MAX is what validation accepts, so opening a plan that started
// before the dropdown's window can still be edited and saved unchanged.
const PLAN_YEAR_BACK = 10;
const PLAN_YEAR_FWD = 15;
const PLAN_YEAR_MIN = 1970;
const PLAN_YEAR_MAX = 2200;
const PLAN_MONTHS_MAX = 120;
const PLAN_AMOUNT_MAX = 99999999;
...
/* ============================================================
   ...
   <input type="month"> is not usable everywhere: browsers without native
   support fall back to a bare text box that happily accepts "2026-13", and the
   ones that DO support it silently blank the field when the typed value is
   invalid. Two selects can only ever produce a real month, look the same in
   every browser, and never lose what was already chosen.
   ============================================================ */
function pad2(n) { return String(n).padStart(2, '0'); }

/* Parse a 'YYYY-MM' key, rejecting month 0/13+ and years outside the editor
   range. Returns { y, m, key, index } or null. */
function parsePlanMonth(key) {
  const mt = /^(\d{4})-(\d{2})$/.exec(String(key || '').trim());
  if (!mt) return null;
  const y = parseInt(mt[1], 10);
  const m = parseInt(mt[2], 10);
  if (!isFinite(y) || !isFinite(m)) return null;
  if (m < 1 || m > 12) return null;
  if (y < PLAN_YEAR_MIN || y > PLAN_YEAR_MAX) return null;
  return { y, m, key: y + '-' + pad2(m), index: y * 12 + (m - 1) };
}

function planMonthShift(key, n) {
  const parsed = parsePlanMonth(key);
  if (!parsed) return '';
  const idx = parsed.index + n;
  return Math.floor(idx / 12) + '-' + pad2((idx % 12) + 1);
}

function monthLabel(m) {
  return (typeof getCurrentLocale === 'function' && getCurrentLocale() === 'en')
    ? MONTH_LABELS_EN[m - 1]
    : m + ' ' + __('plan.monthUnit');
}

/* Year+month selects. `selected` is kept selectable even when it falls outside
   the normal range, so editing an old plan never silently moves its start. */
function monthSelectHtml(selected) {
  const thisYear = new Date().getFullYear();
  const parsed = parsePlanMonth(selected);
  const cur = parsed || parsePlanMonth(currentMonth());
  const years = [];
  for (let y = thisYear - PLAN_YEAR_BACK; y <= thisYear + PLAN_YEAR_FWD; y++) years.push(y);
  if (cur && years.indexOf(cur.y) === -1) { years.push(cur.y); years.sort((a, b) => a - b); }
  const yearOpts = years.map(y =>
    '<option value="' + y + '"' + (cur && y === cur.y ? ' selected' : '') + '>' + y + '</option>').join('');
  let monthOpts = '';
  for (let m = 1; m <= 12; m++) {
    monthOpts += '<option value="' + pad2(m) + '"' + (cur && m === cur.m ? ' selected' : '') + '>' +
      escHtml(monthLabel(m)) + '</option>';
  }
  return '<div class="plan-month-picker">' +
    '<select class="input-field plan-month-year" id="planStartYear">' + yearOpts + '</select>' +
    '<select class="input-field plan-month-mon" id="planStartMonth">' + monthOpts + '</select>' +
    '</div>';
}

/* The chosen start month, or '' when the selects are missing */
function readPlanStartMonth() {
  const y = document.getElementById('planStartYear');
  const m = document.getElementById('planStartMonth');
  if (!y || !m) return '';
  return String(y.value) + '-' + String(m.value);
}

function setPlanStartMonth(key) {
  const parsed = parsePlanMonth(key);
  if (!parsed) return;
  const y = document.getElementById('planStartYear');
  const m = document.getElementById('planStartMonth');
  if (y && !Array.prototype.some.call(y.options, o => o.value === String(parsed.y))) {
    const opt = document.createElement('option');
    opt.value = String(parsed.y);
    opt.textContent = String(parsed.y);
    y.appendChild(opt);
  }
  if (y) y.value = String(parsed.y);
  if (m) m.value = pad2(parsed.m);
  updateEditorPreview();
}
...
function extendPlan(planId) {
  ...
  if (!isFinite(add) || add < 1) { showToast(__('plan.err.months', PLAN_MONTHS_MAX), 'error'); return; }
  const plan = DataStore.getPurchasePlan(planId);
  if (!plan) return;
  // Extending must not push the plan past the same ceiling the editor enforces
  if (plan.months + add > PLAN_MONTHS_MAX) {
    showToast(__('plan.err.extendMax', PLAN_MONTHS_MAX), 'error');
    return;
  }
...
function openPlanEditor(planId) {
    ...
    '<div class="plan-editor">' +
      ...
      '<div class="input-group">' +
        ...
        '<div class="plan-icon-row">' +
          '<input type="text" class="input-field plan-icon-input" id="planIcon" maxlength="8" placeholder="🎯" value="' +
            escHtml(plan ? (plan.icon || MODE_ICON[mode]) : MODE_ICON[mode]) + '">' +
          '<span class="plan-icon-hint">' + __('plan.iconHint') + '</span>' +
      ...
      '</div>' +
      '<div class="flex gap-8 plan-when-row">' +
        '<div class="input-group" style="flex:2;min-width:0">' +
          '<label class="input-label">' + __('plan.fieldStart') + '</label>' +
          monthSelectHtml(plan ? plan.startMonth : currentMonth()) +
          '<div class="plan-month-quick">' +
            '<button type="button" class="btn btn-sm btn-ghost" data-plan-month="0">' + __('plan.monthThis') + '</button>' +
            '<button type="button" class="btn btn-sm btn-ghost" data-plan-month="1">' + __('plan.monthNext') + '</button>' +
          '</div>' +
        '</div>' +
        '<div class="input-group" style="flex:1;min-width:0">' +
          '<label class="input-label">' + __('plan.fieldMonths') + '</label>' +
          '<input type="number" class="input-field" id="planMonths" min="1" max="' + PLAN_MONTHS_MAX + '" step="1" value="' + (plan ? plan.months : 6) + '">' +
        '</div>' +
      '</div>' +
      '<div class="plan-span" id="planSpan"></div>' +
  ...
  ['planStartYear', 'planStartMonth'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.onchange = updateEditorPreview;
  });
  document.querySelectorAll('[data-plan-month]').forEach(btn => {
    btn.onclick = function() {
      setPlanStartMonth(planMonthShift(currentMonth(), parseInt(this.getAttribute('data-plan-month'), 10) || 0));
    };
  });
...
function updateEditorPreview() {
  ...
  // Start → end span, so the period count reads as a real stretch of calendar
  const spanBox = document.getElementById('planSpan');
  if (spanBox) {
    const start = readPlanStartMonth();
    const valid = parsePlanMonth(start);
    if (valid && isFinite(months) && months >= 1 && months <= PLAN_MONTHS_MAX) {
      const end = planMonthShift(start, months - 1);
      spanBox.innerHTML = months === 1
        ? __('plan.spanSingle', valid.key)
        : __('plan.span', valid.key, end, months);
    } else {
      spanBox.innerHTML = '';
    }
  }
...
function savePlanEditor(planId) {
  ...
  const amount = parseFloat((document.getElementById('planAmount') || {}).value);
  const months = Math.floor(parseFloat((document.getElementById('planMonths') || {}).value));
  const startMonth = readPlanStartMonth();
  ...
  if (amount > PLAN_AMOUNT_MAX) { showToast(__('plan.err.amountMax', formatMoney(PLAN_AMOUNT_MAX)), 'error'); return; }
  if (!isFinite(months) || months < 1 || months > PLAN_MONTHS_MAX) { showToast(__('plan.err.months', PLAN_MONTHS_MAX), 'error'); return; }
  // Never trust the field: reject month 0/13+ and out-of-range years outright
  const startParsed = parsePlanMonth(startMonth);
  if (!startParsed) { showToast(__('plan.err.start'), 'error'); return; }
...
function renderPlanOverviewCard(month) {
  ...
  addI18nEntries({
    ...
    'plan.pickIcon': { zh: '常用图标', en: 'Presets' },
    'plan.iconHint': { zh: '可直接输入任意 Emoji', en: 'Type any emoji' },
    'plan.monthUnit': { zh: '月', en: 'month' },
    'plan.monthThis': { zh: '本月', en: 'This month' },
    'plan.monthNext': { zh: '下月', en: 'Next month' },
    'plan.span': { zh: '📆 {0} → {1}（共 {2} 期）', en: '📆 {0} → {1} ({2} periods)' },
    'plan.spanSingle': { zh: '📆 仅 {0} 一期', en: '📆 {0} only (1 period)' },
    ...
    'plan.err.months': { zh: '期数需在 1–{0} 之间', en: 'Periods must be between 1 and {0}' },
    'plan.err.extendMax': { zh: '延长后总期数不能超过 {0} 期', en: 'Total periods cannot exceed {0} after extending' },
    'plan.err.amountMax': { zh: '金额不能超过 {0}', en: 'Amount cannot exceed {0}' },
...
if (document.readyState === 'loading') {
  ...
  window.parsePlanMonth = parsePlanMonth;
  window.planMonthShift = planMonthShift;
  window.readPlanStartMonth = readPlanStartMonth;
  window.setPlanStartMonth = setPlanStartMonth;
