...
function renderWhatIfResults(result) {
  ...
  // Baseline uses actualSpent (my real share only — same scope as totalProjected),
  // NOT getVariableSpending which still includes uncollected split amounts; otherwise
  // all-trend would shift the adjusted total by the unpaid portion.
  ...
  const varTrendTotal = daysPassed > 0 ? actualSpent / daysPassed * daysInMonth : 0;
  ...
  setTimeout(() => drawWhatIfRings(result, adjustedTotal), 50);

  return html;
}

// Draw the comparison rings
// Uses the adjustedTotal already computed in renderWhatIfResults so the ring
// percent always matches the number shown above it (no duplicated/divergent math)
function drawWhatIfRings(result, adjustedTotal) {
  const { income, trendTotal } = result;
  if (adjustedTotal === undefined || adjustedTotal === null) adjustedTotal = trendTotal;
