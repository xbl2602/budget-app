...
**动手改之前先证明它是缺陷。三问，缺一条就别改。**

这条规则是买来的：一次导出/导入审计里报了 19 项「缺陷」，复核后
**2 项是把既定设计当成了 bug，1 项修过了头**。两次栽在同一个坑——
在测试里硬造出一个现实中不可达的数据状态，再把系统在那个状态下的
表现当成缺陷。

**① 这个状态用户真的走得到吗？**

先找**写入点**：`grep` 那个字段，看谁会产生它。

- 反例：「信用卡计划一条扣款流水都没有 → 显示已还清」。真实情况是
  `syncPlanRecords()` 每次启动都会补建、连用户删掉的都会重建，
  **这个状态不可达**，是测试里硬塞出来的。
- 正例：「分摊账单缺 `payer` → 别人欠的钱消失」。查提交历史发现
  过滤代码（`a84bbeb`）先于写入代码（`0ed4503`）落地且无补齐迁移，
  **2026-08-12 之前的账单必然缺它**——历史数据的必然状态，绕不开。

**② 代码里有没有相反的显式不变量？**

同一条规则在多处一致出现，就是设计，不是巧合。

- `planId` 只由 `syncPlanRecords()` 写；「孤儿分期记录必须清除」在
  `savePlanEditor`（切走 credit 时）、`deletePurchasePlan`、`repairData`
  三处一致 —— 改其中一处就是在捅洞。
- `repairData` 清空非 credit 计划的 `categoryId`，对应的是
  `savePlanEditor` 里的 `categoryId: mode === 'credit' ? categoryId : ''`。

**③ 有没有历史决策记录说过相反的话？**

`technical/` 与 `docs/` 下有成套的技术记录与审计报告（`technical/` 在
`.gitignore` 里，只在本机）。改动前先 `grep` 一遍：

- `technical/purchase-plans-audit-technical-20260815.md` 明写
  credit 台账不回看真实流水是**「不能双重计算」的正确设计**；
  也判定 `repairData` 处理孤儿流水「修复正确且零误伤」。
- 同一份报告独立确认过 `allTags`/`tagColors` 同步丢失、CSV 缺
  `planId` 列确实是缺陷 —— 它两个方向的结论都可用。

**推翻自己的论据也要一样严格。** 我当时的理由是「AI 导入会产生带
`planId` 的孤儿记录」，翻开 `docs/ai-data-import-spec.md` §3.2 才发现
records 白名单只有 8 个字段、根本没有 `planId`——**前提是假的**。

**配套心态：**
- ❌ 「测试能复现」不等于「用户会遇到」——先问状态可达性
- ❌ 顺手把修复扩成新功能（撤销窗口那次多加了跨会话恢复，结果把
  `142ea5c` 专门修掉的「pending 卡住」请了回来）
- ❌ 拿转录/复制的代码副本当验证依据（`verify-lan-sync-direct.js`
  自述 `verbatim transcription`，跑的是旧副本，改了也不会变绿）
- ✅ 验证必须驱动**真实代码路径**，并留一个造不了假的佐证
