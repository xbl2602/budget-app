...
> **范围**：仅消费流水（`records` + `categories`）。**不**生成预算、收入、储蓄目标、账单分类、分摊账单。
...
- 约束：
  ...
  - **不生成** `budgets`、`monthlyIncome`、`savingsTarget`、`billCategories`、`billAmounts`、`splitBills`、`contacts` 等键
...
5. **「🔄 替换当前数据」仅在完全重开时使用**：replace 会用文件内容**整体覆盖**数据，`categories` 只会剩下文件里的分类——若文件只含新建分类，29 个默认分类将全部丢失，已有流水会变「未知分类」。替换模式下 AI 必须输出**完整分类表**（§4 全部 29 个默认分类 + 新建分类）后再交付
6. 导入成功后刷新页面检查流水/统计
